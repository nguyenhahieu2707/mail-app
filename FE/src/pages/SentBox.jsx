...
import { getSent, getMailDetail } from '../services/mailApi';
import { FiUser, FiSend, FiPaperclip } from 'react-icons/fi';
    ...
    return (
        <div className="email-detail p-4">
            ...
            <div className="d-flex align-items-center mb-4">
                ...
                <div>
                    <h6 className="mb-0 text-truncate">Đến: {email.to}</h6>
                    <small className="text-muted">từ tôi</small>
                </div>
        ...
        <div className="row g-0">
            <div className="col-md-4 border-end vh-100 overflow-auto">
                <div className="p-3 border-bottom sticky-top bg-light">
                    <h5 className="mb-0">Thư đã gửi</h5>
                </div>
                {isLoading && <LoadingSpinner />}
                {error && <ErrorMessage message={error} />}
                {!isLoading && !error && (
                    <div className="list-group list-group-flush">
                        {sentEmails.length > 0 ? sentEmails.map(email => (
                            <div
                                key={email.id}
                                className={`list-group-item list-group-item-action email-list-item p-3 ${selectedEmailId === email.id ? 'active' : ''}`}
                                onClick={() => handleSelectEmail(email.id)}
                            >
                                <div className="d-flex w-100 justify-content-between">
                                    <p className="mb-1 text-truncate">Đến: {email.to}</p>
                                    <small className="text-nowrap">{new Date(email.date).toLocaleDateString('vi-VN')}</small>
                                </div>
                                <p className="mb-1 text-truncate">{email.sub}</p>
                                <small className="text-muted text-truncate d-block">{email.body?.substring(0, 80) ?? ''}...</small>
                            </div>
                        )) : (
                            <p className="p-3 text-muted">Không có thư đã gửi nào.</p>
                        )}
                    </div>
                )}
            </div>
            ...
            <div className="col-md-8 vh-100 overflow-auto">
                <EmailDetailView emailId={selectedEmailId} type="sent" />
            </div>
        </div>
    );
}
